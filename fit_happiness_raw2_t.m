function [fitoutput] = fit_happiness_raw2_t(inx,lb,ub, data,fitoutput_choice)
...
for i=1:length(fitoutput.outcome)
    ...
    elseif fitoutput.choice(i,1)==2
        fitoutput.CR(i,1)=0;
        fitoutput.GR(i,1)=fitoutput.outcome(i,1);
...
for m=1:length(rating_ind)
    ...
    GR_mtx(m,1:length(1:rating_ind(m))) = fliplr(transpose(fitoutput.GR(1:rating_ind(m))));
...
[b, ~, ~, ~, ~, ~, H] = fmincon(@model, inx, [],[],[],[],lb,ub,[], options, CR_mtx, GR_mtx, rating,rating_ind);
    fitoutput.b  = b;
    fitoutput.se = transpose(sqrt(diag(inv(H)))); %does not always work
    [sse, happypred, happyr2] = model(b, CR_mtx, GR_mtx, rating,rating_ind);
...
function [sse, happypred, happyr2] = model(x, CR_mtx, GR_mtx, rating,rating_ind)
weight_CR = x(1);
weight_GR = x(2);
%weight_RPE = x(3);
gamma = x(3);
...
happypred = weight_CR*CR_mtx*dec + weight_GR*GR_mtx*dec + weight_t*rating_ind' + const;
% for i =1:length(CR)
