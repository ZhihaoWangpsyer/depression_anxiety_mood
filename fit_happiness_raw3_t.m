function [fitoutput] = fit_happiness_raw3_t(inx,lb,ub, data,fitoutput_choice)
...
decayvec1  = gamma1.^[0:(size(CR_mtx,2)-1)]; 
decayvec1  = decayvec1(:);
dec1       = decayvec1;

decayvec2  = gamma2.^[0:(size(CR_mtx,2)-1)]; 
decayvec2  = decayvec2(:);
dec2       = decayvec2;

decayvec3  = gamma3.^[0:(size(CR_mtx,2)-1)]; 
decayvec3  = decayvec3(:);
dec3       = decayvec3;

happypred = weight_CR*CR_mtx*dec1 + weight_EV*EV_mtx*dec2 + weight_RPE*RPE_mtx*dec3 + weight_t*rating_ind' + const;
% for i =1:length(CR)
