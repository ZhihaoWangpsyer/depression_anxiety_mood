function [fitoutput] = fit_happiness_raw5_t(inx,lb,ub, data,fitoutput_choice)
...
for i=1:length(fitoutput.outcome)
    if fitoutput.choice(i,1)==1
        if fitoutput.gamble_1(i,1) > fitoutput.gamble_2(i,1)
            fitoutput.diff(i,1) = fitoutput.certain(i,1) - fitoutput.gamble_1(i,1);
        else
            fitoutput.diff(i,1) = fitoutput.certain(i,1) - fitoutput.gamble_2(i,1);
        end
    elseif fitoutput.choice(i,1)==2
        if fitoutput.type(i,1)==1 && fitoutput.outcome(i,1)>0
           fitoutput.diff(i,1) = 0;  
        elseif fitoutput.type(i,1)==1 && fitoutput.outcome(i,1)<0
            fitoutput.diff(i,1) = fitoutput.certain(i,1) - min(fitoutput.gamble_1(i,1),fitoutput.gamble_2(i,1)); 
        elseif fitoutput.type(i,1)==2 && fitoutput.outcome(i,1)>0
            fitoutput.diff(i,1) = 0;  
        elseif fitoutput.type(i,1)==2 && fitoutput.outcome(i,1)==0   
            fitoutput.diff(i,1) = fitoutput.certain(i,1) - min(fitoutput.gamble_1(i,1),fitoutput.gamble_2(i,1)); 
        elseif fitoutput.type(i,1)==3 && fitoutput.outcome(i,1)==0
            fitoutput.diff(i,1) = 0;  
        elseif fitoutput.type(i,1)==3 && fitoutput.outcome(i,1)<0   
            fitoutput.diff(i,1) = fitoutput.certain(i,1) - min(fitoutput.gamble_1(i,1),fitoutput.gamble_2(i,1)); 
        end
...
for m=1:length(rating_ind)
    ...
    diff_mtx(m,1:length(1:rating_ind(m))) = fliplr(transpose(fitoutput.diff(1:rating_ind(m))));
end
...
[b, ~, ~, ~, ~, ~, H] = fmincon(@model, inx, [],[],[],[],lb,ub,[], options, CR_mtx, EV_mtx, RPE_mtx, diff_mtx,rating,rating_ind);
    fitoutput.b  = b;
    fitoutput.se = transpose(sqrt(diag(inv(H)))); %does not always work
    [sse, happypred, happyr2] = model(b, CR_mtx, EV_mtx, RPE_mtx, diff_mtx,rating,rating_ind);
...
function [sse, happypred, happyr2] = model(x, CR_mtx, EV_mtx, RPE_mtx, diff_mtx,rating,rating_ind)
weight_CR = x(1);
weight_EV = x(2);
weight_RPE = x(3);
weight_diff = x(4);
...
happypred = weight_CR*CR_mtx*dec + weight_EV*EV_mtx*dec + weight_RPE*RPE_mtx*dec + weight_diff*diff_mtx*dec + weight_t*rating_ind' + const;
% for i =1:length(CR)
