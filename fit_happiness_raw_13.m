function [fitoutput] = fit_happiness_raw_13(inx,lb,ub, data,fitoutput_choice)
...
fitoutput.CR_utility = fitoutput_choice.CR_utility;
fitoutput.EV_utility = fitoutput_choice.EV_utility;
fitoutput.GR_utility_diff = fitoutput_choice.GR_utility_diff;
...
for m=1:length(rating_ind)
    CR_mtx(m,1:length(1:rating_ind(m))) = fliplr(transpose(fitoutput.CR_utility(1:rating_ind(m))));
    EV_mtx(m,1:length(1:rating_ind(m))) = fliplr(transpose(fitoutput.EV_utility(1:rating_ind(m))));
    GR_diff_mtx(m,1:length(1:rating_ind(m))) = fliplr(transpose(fitoutput.GR_utility_diff(1:rating_ind(m))));
end
...
[b, ~, ~, ~, ~, ~, H] = fmincon(@model, inx, [],[],[],[],lb,ub,[], options, CR_mtx, EV_mtx, GR_diff_mtx, rating,rating_ind);
    fitoutput.b  = b;
    fitoutput.se = transpose(sqrt(diag(inv(H)))); %does not always work
    [sse, happypred, happyr2] = model(b, CR_mtx, EV_mtx, GR_diff_mtx, rating,rating_ind);
...
function [sse, happypred, happyr2] = model(x, CR_mtx, EV_mtx, GR_diff_mtx, rating,rating_ind)
weight_CR = x(1);
weight_EV = x(2);
weight_RPE = x(3);
gamma = x(4);
...
happypred = weight_CR*CR_mtx*dec + weight_EV*EV_mtx*dec + weight_RPE*GR_diff_mtx*dec + weight_t*rating_ind' + const;
% for i =1:length(CR)
